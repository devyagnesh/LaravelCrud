...
<body>
    <form action="{{ route('postEditProduct', ['id' => $product['id']]) }}" method="POST" enctype="multipart/form-data"
        class="w-3/6 block mx-auto my-6">
        @csrf
        @if (session('success'))
        ...
        <div class="mb-4">
            ...
            <select id="category" name="category"
                ...
                @foreach ($categories as $category)
                    <option value="{{ $category['id'] }}" @if ($category['id'] == $product['cateid']) selected @endif>
                        {{ $category['name'] }}
                    </option>
                @endforeach
            </select>
        ...
        <button type="submit"
            class="w-full bg-indigo-500 text-white py-2 rounded-md hover:bg-indigo-600 transition duration-300">Edit
            Product</button>
    </form>
</body>
